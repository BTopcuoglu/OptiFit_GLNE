...
get_testing_feats <- function(method,split){
...
get_vals <- function(method,split){
    
    train_feats <- get_training_feats(method,split)
    test_feats <- get_testing_feats(method,split)
    
    #subset to features that are in the training set
    sub_test_feats <- test_feats[test_feats %in% train_feats]
    
    return(tibble(train=length(train_feats),
                  test=length(test_feats),
                  fract_test_in_train=test/train))
} 
...
data <- expand_grid(methods,splits)  %>% 
    ...
    summarize(get_vals(methods,splits),
              .groups = "drop")

write_csv(data,"results/tables/n_features.csv")