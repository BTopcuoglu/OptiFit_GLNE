...
# allData <- bind_rows(dataTransformed %>% mutate(grps = rep("train",nrow(.))),
#                      testTransformed %>% mutate(grps = rep("test",nrow(.))))  %>% 
#     select(dx,grps,everything())
allData <- bind_rows(dataTransformed,testTransformed)
    
#grps <- c(rep("TR",nrow(dataTransformed)),rep("TE",nrow(testTransformed)))
grps <- c(paste0("TR",seq(1,nrow(dataTransformed))),
          paste0("TE",seq(1,nrow(testTransformed))))
...
results <- mikropml::run_ml(dataset = allData,
                            ...
                            group_partitions = list(train = c(paste0("TR",seq(1,nrow(dataTransformed)))),
                                                    test = c(paste0("TE",seq(1,nrow(testTransformed))))),
                            seed = 2021)
...
write_csv(results$performance,file = paste0(outDir, "cv_results_", split, ".csv"))
